    ...
    def forward(self,
        ...
        Args:
            mlvl_feats (list(Tensor)): Input queries from
                ...
                [bs, embed_dims, h, w]. 2d图像特征
            query_embed (Tensor): The query embedding for decoder,
                with shape [num_query, c].  [num_query, 256]
        ...
        # 全连接预测出在BEV空间的3D reference point offset坐标(x, y, z)， 
        # query_pos: [1, 900, 256]--->reference_points：[1, 900, 3]
        ...
        # inter_states： 3d参考点在fpn的采样特征， inter_references：修正后的3d参考点
...
class Detr3DTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self,
        ...
        # 每层layer之中，所有的object query之间做self-attention来相互交互获取全局信息并避免多个query收敛到同个物体。
        # object query再和图像特征之间做cross-attention
...
def feature_sampling(mlvl_feats, reference_points, pc_range, img_metas):
    ...
    # 左乘转换矩阵转图片坐标,获得三维点在相机投影点, reference_points_cam: [1, 6, 900, 4, 1]--->[1, 6, 900, 4]
    ...
    # 对不在grid内的点，也就是投影在某个cam以外的点进行mask
    ...
    mask = torch.nan_to_num(mask)
    sampled_feats = []
    # 对四个特征层分别求出线性插值后的feature
    for lvl, feat in enumerate(mlvl_feats):
        B, N, C, H, W = feat.size()
        feat = feat.view(B*N, C, H, W)
        reference_points_cam_lvl = reference_points_cam.view(B*N, num_query, 1, 2)
        # 利用F.grid_sample去feat上采样指定位置的点
